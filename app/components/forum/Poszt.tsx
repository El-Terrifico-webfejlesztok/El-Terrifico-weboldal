  ...
  return (
    <div className="collapse max-w-screen-2xl collapse-arrow bg-base-300 sm:w-5/6 w-full mx-auto sm:mb-3 mb-5">
      ...
      <div className="collapse-title text-xl font-medium flex justify-between items-center">
        {/** Cím bal odal */}
        <div className="sm:mr-0 max-w-[60%]">
          {/** Cím */}
          <p className="text-base text-left sm:text-lg">{post.title}</p>
          {/** Kategória és dátum */}
          <div className="flex">
            <div className=" text-xs sm:text-sm text-info font-normal">#{post.category}</div>
            <p className="ml-2 text-left font-light text-xs sm:text-sm">{postdate}</p>
            {post.comments.length > 0 ?
              <p className="mx-3 font-light text-xs sm:text-sm flex ">
        ...
        <div className="flex items-center">
          {post.user.username === session?.user?.name || session?.user?.role === 'admin' ?
            <button onClick={() => deletePost(post.id)} className="z-20 btn btn-sm btn-circle bg-warning btn-outline sm:mr-5 mr-2">
